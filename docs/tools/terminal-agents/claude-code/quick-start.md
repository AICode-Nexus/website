...
- 你想把终端和仓库本身当成主入口。
- 你愿意先写清边界、命令和验收，再让 agent 动代码。
- 这次任务更像 repo pairing，而不是平台委派或 IDE 工作台协作。
...
第一次 quick start 不求“改得多”，只求：

- 在仓库根目录把任务合同写清楚。
- Claude Code 先复述边界，再做一轮最小改动。
- 你手里有命令结果和一段简短交付说明。
...
第一次最好再补一行“交付物”，例如“输出一段变更说明和命令结果”。
...
```text
请先复述任务、范围、禁止修改区和验收命令。
然后给出最小计划，只推进第一轮最小改动。
完成后必须回报真实命令结果，并总结改了什么、没改什么、还有什么风险。
```
...
第一次 quick start 不要只留下 diff。至少收口这三样东西：
...
- 没写任务合同就直接开跑，结果 agent 自己补全边界。
- 只看改动，不看命令结果，最后不知道仓库到底有没有验证过。
- 第一次就选长链路 feature，导致终端闭环变成漫长对话。
- 把 Claude Code 当成平台层工具使用，期待它自动解决 PR 流和多人交接。
