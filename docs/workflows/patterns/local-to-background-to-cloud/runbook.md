...
| 阶段 | 要做什么 | 产出 |
| --- | --- | --- |
| Local discovery | 读代码、跑命令、缩边界。 | 结构化 brief。 |
| Background execution | 推进已收敛部分并保留日志。 | diff、日志、验证结果。 |
| Platform close | 审阅、交接、最终判断。 | PR 或任务系统收口记录。 |
...
5. 把本轮关键信息写回 PR、issue 或仓库文档，避免只留在面板里。
...
- 本地阶段还说不清边界，先停，不要往后台丢模糊问题。
- 后台只返回结论不返回证据，先停，补日志和 diff。
- 平台阶段拿不到可 review 资产，先停，回前两段补材料。
...
- brief 是否真的让别人可以接手下一段。
- 后台结果是否能脱离会话继续复盘。
- 平台收口是否承载了最终判断，而不是只展示状态。
- 如果明天换 owner，是否仍能看懂当前做到哪一步。
